(() => {
  class UploaderController {
    ...
    load() {
      this.loadingPromise = this.UploaderService.getDocuments(this.resolve.id).then((response) => {
    ...
    loadTags(query) {
      ...
      return this.UploaderService.listTags(query).then((response) => {
    ...
    save(data, field, id) {
      ...
      this.UploaderService.update(aux.payload(field, data), id, true).then(
        (response) => {
          this.getFolders();
        },
        (response) => {
    ...
    base(file) {
      this.Upload.base64DataUrl(file).then((response) => {
  ...
  angular.module('app').component('uploaderController', {

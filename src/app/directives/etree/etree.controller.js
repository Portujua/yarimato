(() => {
  class ETreeController {
    constructor($scope, ETreeService, $attrs, $timeout) {
      ...
      */
      ...
      // Toggle filter input with scope value (from outside)
      $scope.$watch('showFilter', (data) => {
    ...
    _toggle(node, fn, param) {
      try {
        this.toggleFn({ node: node }).finally(() => {
          fn(param);
        });
      } catch (e) {
    ...
    _remove(node) {
      try {
        this.removeFn({ node: node }).finally(() => {
          this.$timeout(() => {
            delete this.filter;
            this.doFilter();
          }, 100);
  ...
  angular.module('app').controller('ETreeController', ETreeController);
})();

...
angular
  ...
  .filter('available', () => {
    return (input, attr) => {
      ...
      return _.isNull(input) || s.isBlank(input) || _.isUndefined(input) || _.isNaN(input) || input === 'NaN' ? attr : input;
    };
  })
  ...
  .filter('defaultCatalogImage', () => {
    return (input, attr) => {
      ...
      return _.isNull(input) || s.isBlank(input) || _.isUndefined(input) || input.substr(-4) === 'null' ? attr : input;
    };
  })
  ...
  // => "Helloworld"
  .filter('removeSpaces', () => {
    return (str, original = ' ', destiny = '') => {
      return _(str).isString() ? str.replace(new RegExp(original, 'ig'), destiny) : str;
    };
  })

(() => {
  angular.module('app').directive('collapsibleText', ($compile) => ({
    restrict: 'A',
    link: ($scope, element, attrs, api) => {
      ...
      attrs.$observe('collapsibleText', (text) => {
        ...
        if (text.length > maxLength) {
          ...
          let toggleButton = $compile(
            '<a href="" class="collapse-text-toggle" ng-click="toggle()">({{ collapsed ? "less" : "more" }})</a>'
          )($scope);
